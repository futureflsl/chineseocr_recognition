...
class PathDataset(Dataset):
    def __init__(self, txt_file, data_dir='./data/images', transform=None, target_transform=None):
        """
        加载本地目录图片
        """
        ...
        self.paths = []
        self.labels = []
        lines = self.load_lines(os.path.join(txt_file))
        for line in lines:
            data = line.split('\t')
            self.paths.append(data[0])
    ...
    def __getitem__(self, index):
        ...
        imP = os.path.join(self.data_dir, self.paths[index])
